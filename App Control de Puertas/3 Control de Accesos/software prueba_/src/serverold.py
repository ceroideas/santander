...
def _connect_board(board_id: int) -> bool:
    ...
    try:
        ...
        if ok:
            ...
            add_event("OK", f"Conectado a {cfg['host']}:{cfg['port']}", board_id)
        else:
            io_state[board_id]["connected"] = False
            io_state[board_id]["error"] = "No se pudo establecer conexión TCP"
            add_event("ERR", f"Fallo al conectar con {cfg['host']}:{cfg['port']}", board_id)
        return ok
